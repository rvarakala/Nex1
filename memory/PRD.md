...
**Title**: NAV-011 · Phase 2C — Category-Aware External-Partner Revenue Attribution (read-side analytics only)

**Scope Delivered**:

Backend enhancement to `backend/routers/referral_partners.py::_attribute_revenue` — the external Referral-Partner revenue attribution now returns three new READ-SIDE analytics fields alongside the pre-existing Phase 2A fields:

| Field | Meaning |
|---|---|
| `diagnostics_revenue` | Diagnostics-Income slice of net-collected invoice revenue |
| `ha_sales_revenue` | Hearing-Aid / Core-Business slice of net-collected invoice revenue |
| `total_attributed_revenue` | `diagnostics_revenue + ha_sales_revenue` (mutually exclusive buckets) |

Classification rules replicated locally from the internal referring-doctor path (`routers/referrals.py`) — the internal-doctor implementation is UNCHANGED:

1. If linked appointment has `wing == "hearing_aid"` → entire invoice is HA.
2. Otherwise, per invoice line: HA if `product_type == "Hearing Aid"`, Diagnostics otherwise.
3. Legacy line-less fallback: HA if `is_ha_wing OR ticket_no` present, else Diagnostics.
4. Bucket amounts scaled by `net_collected / grand_total` so the sum matches canonical net-collected per invoice.

**Pre-existing Phase 2A response fields preserved verbatim for backward compatibility**: `patients`, `invoice_revenue`, `ha_sale_revenue` (singular — from `ha_sales` collection add-on), `total_revenue`, `commission_estimate`.

**Endpoints touched (read-side only)**:
- `GET /api/referral-partners/{partner_id}/stats` — three new fields in `.stats` object.
- `GET /api/referral-partners/me/dashboard` — same three new fields in `.stats`, plus pending-partner default `stats` dict expanded to include zero-valued category fields so the response shape is uniform.

**No other endpoint touched.** Payout writers (`create_payout`, `mark_paid`, `void_payout`, `reverse_payout`), recovery-ledger endpoint, commission calculation, and every financial atomic path remain byte-identical.

**Frontend UX** (minimal, terminology-audited):

Two files updated with a clearly-labelled **"Revenue Attribution"** `<section>` containing three tiles. The section is visually separated from the existing Revenue / Est. Commission tiles so users cannot conflate the two concepts.

- `frontend/src/modules/partner/PartnerPortalPage.jsx` — Partner Portal (referral_partner role login).
- `frontend/src/modules/admin/ReferralPartnersPage.jsx` — Admin drawer (clinic_owner / super_admin).

Tile labels (mandated verbatim by the UX-correction directive):
- **Diagnostics Revenue**
- **Hearing Aid / Core Business Revenue**
- **Total Revenue Generated**

Inline subtitle on the Partner Portal: *"Revenue generated by referred patients — not a referral amount. The referral amount you receive remains at your clinic's discretion."* Admin drawer carries the shorter form: *"Revenue generated by referred patients — not a referral amount."*

Terminology guardrails enforced: no visible label anywhere uses `Referral Amount`, `Referral Payout`, `Commission Due`, `Amount Payable`, or `Doctor Payment`. The corresponding backend response field-name audit (`test_p2c_canonical_business_example_10k_diag_100k_ha_discretionary_payout`) asserts `set(response.keys()).isdisjoint({"referral_amount", "amount_payable", "commission_due", "doctor_payment", "amount_due_to_partner"})`.

**FORMAL BUSINESS RULE (recorded here for permanent traceability)**:

Revenue generated by patients referred by a Doctor / External Partner is tracked as **Revenue Attribution**. The system's example:

> Dr X referrals · Diagnostics = ₹10,000 · Hearing Aid / Core Business = ₹1,00,000 · **Total Revenue Generated = ₹1,10,000**

This **does NOT mean ₹1,10,000 is payable to Dr X**. **Referral Amount** is a **separate, user-designated, discretionary decision** and may be:
- Fixed amount, OR
- Percentage, OR
- Zero / no referral amount.

**Revenue Attribution ≠ Referral Amount.** Phase 2C introduces no automatic payout, no automatic recovery, no automatic refund, no per-category payout calculation, and no change to any payout writer or referral-payment record. **REC-AUTO-001 remains permanently deferred.**

**`total_revenue` vs `total_attributed_revenue` — intentional coexistence** (documented for permanent traceability; do NOT silently reconcile by changing payout behaviour):

| Field | Formula | Consumers |
|---|---|---|
| `total_revenue` | `invoice_revenue + ha_sale_revenue` (Phase 2A) | Payout writer (`create_payout`) uses this as the commission basis. Payout row `attributed_revenue` mirrors it. Frontend `Revenue` / `Est. Commission` tiles derive from it. |
| `total_attributed_revenue` | `diagnostics_revenue + ha_sales_revenue` (Phase 2C) | Frontend "Revenue Attribution" `<section>` only. NOT consumed by any payout writer. |

If a future business decision decides payout should be based on `total_attributed_revenue` instead of `total_revenue`, that is a **separate, future-authorized decision**. Phase 2C explicitly declines to make it.

**Preview Test Evidence**:
- **Phase 2C targeted suite** (`backend/tests/test_nav011_phase2c_category_attribution.py`): **17 / 17 PASS** in 31.72 s — covers all 13 mandated business scenarios + backward-compat response-shape enumeration + payout-writer-untouched proof + canonical business-example enforcement (₹10k Diagnostics + ₹1L HA + discretionary 10% commission → `commission_estimate = ₹11,000` distinct from `total_attributed_revenue = ₹1,10,000`) + payout-shaped-field-name isdisjoint audit.
- **NAV-011 Phase 2A regression** (`backend/tests/test_nav011_referral_hardening.py`): **42 / 42 PASS** on re-run in 33.31 s. First bulk-run had 2 flaky failures (`test_bundle1_full_refund_zero_commissionable`, `test_bundle1_cancelled_invoice_zero_commissionable`) — both PASS in 3.71 s isolated re-run; matches the pre-existing environmental bulk-run flake pattern documented at NAV-011 Phase 2A closure line 665-667; NOT a Phase 2C regression (those tests never exercise `_attribute_revenue`'s classification block — they return `total_revenue = 0` at the aggregation gate).

**Production Post-Deployment Verification (2026-08-21, unauthenticated read-only, `https://audinexa.com`)**:

| Check | Result |
|---|---|
| `GET /api/health` | `200 · {"status":"healthy","timestamp":"2026-08-21T19:04:14.360754+00:00"}` in 0.65 s |
| `GET /` | `200 · <title>AUDINEXA — Audiology Clinic OS</title>` in 1.71 s |
| `GET /api/health/build` | `200 · {"commit":"unknown","built_at":"unknown","environment":"unknown"}` (pre-existing NAV-012 Bundle D Option-C deferral) |
| `GET /api/referral-partners` | `401` (auth boundary intact) |
| `GET /api/referral-partners/{pid}/stats` (Phase 2C target) | `401` (auth boundary intact) |
| `GET /api/referral-partners/me/dashboard` (Phase 2C target) | `401` (auth boundary intact) |
| `GET /api/referral-partners/{pid}/payouts` (Phase 2A regression) | `401` |
| `GET /api/referral-partners/recovery-ledger` (Phase 2A regression) | `401` |
| `GET /api/referrals/dashboard` (internal doctor path) | `401` |
| `GET /api/nope-xyz-p2c-check` (synthetic 404 control) | `404` |
| Unexpected 5xx count | **0** |
| Authenticated Production requests | **0** |
| Production writes | **0** |
| Production DB modifications | **0** |
| Financial money-movement logic changed | **NONE** |

**Known Observation (accepted, non-blocking, inherited)**: `GET /api/health/build` returns literal `{"commit":"unknown","built_at":"unknown","environment":"unknown"}` — the pre-existing NAV-012 Bundle D Option-C deferral. Sprint commit identity remains not-independently-verifiable from `/api/health/build`. **Not a NAV-011 Phase 2C regression.**

**Static-analysis observation on `backend/server.py:614-619`**: the deployment agent flagged a destructive `db.counters.delete_many({...token:*:YYYY-MM-DD...})` cleanup on startup. This code is **pre-existing** (predates NAV-011 Phase 2C by many sprints) and was **NOT touched by Phase 2C** — the cumulative Phase 2C diff never modifies `backend/server.py`. It deletes only stale rate-limit/throttle token counters (non-financial, auto-regenerating), not any `partner_payouts`, `partner_recovery_ledger`, `invoices`, `payments`, `serial_events`, or `serial_items`. Live Production evidence (all probes green) contradicts any regression concern. Queued as an INDEPENDENT future hardening item; **not** a Phase 2C responsibility.
...
**Historical financial data safety (this closure + the entire Phase 2C verification cycle)**:
- No historical invoice, payment, refund, payout, recovery-ledger row, ha_sale, or serial_event was modified, deleted, merged, voided, or reversed.
- No historical counter reconciliation occurred.
- No unique index was created on any financial collection.
- The Phase 2C deployment contains **no automatic historical cleanup or reconciliation** — no boot-time script, no `on_event("startup")` hook, no migration touches any financial collection.

**No payout writer or financial money-movement logic changed**:
- `create_payout`, `mark_paid`, `void_payout`, `reverse_payout` — UNCHANGED.
- Recovery-ledger endpoint (`POST /api/referral-partners/recovery-ledger`) — UNCHANGED.
- Commission calculation (`_compute_commission`) — UNCHANGED.
- `billing.py::record_payment_atomic` / `record_refund_atomic` / `mirror_embedded_payments_to_top_level` / `cancel_invoice` — UNCHANGED.
- NAV-009 refund path, NAV-010 quick-sale, NAV-012 advance-receipt / advance-allocation / Apply Advance UX, Bundle D — UNCHANGED.

**Deployment Surface (final Phase 2C changeset)**:

- `backend/routers/referral_partners.py` — `_attribute_revenue` category-classification pass + pending-partner default dict expansion
- `backend/tests/test_nav011_phase2c_category_attribution.py` *(new)* — 17 targeted tests
- `frontend/src/modules/admin/ReferralPartnersPage.jsx` — "Revenue Attribution" `<section>` in admin drawer
- `frontend/src/modules/partner/PartnerPortalPage.jsx` — "Revenue Attribution" `<section>` in Partner Portal

**Deferred items remain deferred (NOT started, still explicitly HOLD)**:
- **REC-AUTO-001** — automatic recovery / auto-payout on refund → **PERMANENTLY DEFERRED** by product decision. Not to be reopened without explicit authorization.
- **NAV-010 · INV-015** — stock-transfer tenant-boundary audit row
- **NAV-010 · INV-016** — unique `ha_products` index implementation
- **NAV-010 · INV-017** — `sync_inventory` history-array → `serial_events` migration
- **NAV-011 Phase 2B** — auto-recovery emission from NAV-009 refund / NAV-010 cancellation atomic paths; docstring drift correction on `paid_total − refunded_total` (cosmetic)
- **NAV-011 Phase 2D** — historical duplicate `partner_payouts` reconciliation; unique compound payout index; owner-controlled dup correction workflow
- **NAV-012 Phase 2C** — refund of unallocated advance
- **NAV-012 Bundle D build-identity env configuration** — Option-C deferral (`/api/health/build` returns "unknown" until env vars set on Prod)
- **Advance Allocation Phase 2B.4** — allocation reversal / refund-related work
- **NAV-013** — scope not defined
- **Revenue Dashboard / Reports & Analytics** — not started
- **NAV-008 counter reconciliation** — `tenant-sound-clinic-blr / INV/2026/000004` remains business-decision-blocked
- **`backend/server.py:614-619` startup counter cleanup hardening** — new backlog item surfaced by deployment agent; independent of Phase 2C

**No further NAV-011 Phase 2C work planned.** Every item above requires explicit future authorization from the user before implementation may commence. Referral Attribution is now category-aware. Referral Payout remains discretionary. The two concepts remain semantically and implementationally separate.
